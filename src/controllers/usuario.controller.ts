...
export class UsuarioController extends BaseController {
  ...
  async register(request: FastifyRequest, reply: FastifyReply): Promise < void> {
    try {
      const { nome, email, senha, tipo, empresaId } = request.body as RegisterRequest

      if(tipo === 'USUARIO' && !empresaId) {
    return reply.status(400).send({
      error: 'empresaId é obrigatório para usuários do tipo USUARIO'
    })
  }
...
// Gerar token JWT com tipo e empresaId
...
// Para USUARIO, expõe o empresaId em cookie acessível ao frontend
if (user.tipo === 'USUARIO' && user.empresaId) {
  reply.setCookie('empresaId', user.empresaId, {
    ...cookieOptions,
    httpOnly: false,
  })
}
...
reply.send({
  ...
  try {
    const empresaId = this.getEmpresaFilter(request)
      ...
      reply.send({ message: 'Usuários encontrados', data: formattedData })
